...
5. Load the global `analyze-sessions` skill and select up to five most recent top-level Pi sessions started within the last 14 days whose recorded canonical `cwd` exactly equals `pwd -P`. The selected set must include the current session; exclude nested subagent transcripts from the five-session count. Session CLI `--cwd` is a substring filter, so verify exact equality from session metadata before inclusion. If the current session cannot be identified or read, stop and report the limitation without writing anything. If fewer than five matching sessions exist, analyze all matches; do not broaden scope.
...
5. For session-derived items, cite the sanitized session short ID, date, and evidence theme without exposing unrelated transcript content. Then verify the need against current project files/configuration.
