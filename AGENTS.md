...
- Before adopting an important technical decision, delegate one read-only review to `architector`. Important decisions include system or cross-component architecture, public APIs or protocols, data models or migrations, security boundaries, hard-to-reverse dependencies, and choices among materially different approaches with significant long-term cost or risk. Give it the decision context, constraints, evidence, and viable alternatives; use its result as advisory input and remain responsible for the final decision. Do not invoke it for routine, local, or easily reversible implementation choices.
- When delegating implementation that may encounter an important technical decision, either obtain the `architector` review first or explicitly authorize the worker to spawn at most one `architector` child, with maximum descendant depth 1 and direct-child fan-out 1 for that review only.
- When you are the coordinator, after completing a candidate implementation and initial verification, delegate exactly one independent read-only review to the resolved `reviewer` profile for changes with material risk, such as security-sensitive code, concurrency or lifecycle behavior, data or schema migrations, automation with external side effects, production-facing behavior, or substantial cross-file interactions. Skip routine, local, easily reversible changes and changes governed by a more specific review rule, including the pre-edit `AGENTS.md` review above. This post-implementation review complements and does not replace any required pre-decision `architector` review. Before dispatch, confirm that the resolved profile has only read-only tools and no `subagent_agents`; project profiles may override the global profile. Give the reviewer the complete task requirements, candidate diff or the path to a complete readable diff artifact, and verification results. The reviewer must not be the implementation agent and must not edit files, run mutating commands, commit, push, or delegate. Run this pass once per qualifying task; assess and resolve or explicitly document material findings, then perform final verification yourself. The coordinator remains responsible for the result.
